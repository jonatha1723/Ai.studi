...
export default function ImageUploader({ 
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      for (const file of uniqueFiles) {
        ...
        let docRefPrimary;
        try {
          docRefPrimary = await addDoc(collection(dbPrimary, 'images'), {
            userId: user.uid,
            ciphertext,
            iv,
          ...
          handleFirestoreError(error, OperationType.WRITE, 'images');
          throw error;
        }
  ...
  React.useEffect(() => {
    if (initialFiles.length > 0 && !uploading) {
      onDrop(initialFiles);
    }
  }, [initialFiles, onDrop, uploading]);
